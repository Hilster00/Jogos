		...
		for(int i=0;i<4;i++) {
			for(int j=0;j<4;j++) {				
				...
				if((i%2==0 && j%2==0)||(i%2!=0 && j%2!=0) ) {
					peca.setIcon(new ImageIcon(tela.class.getResource("/imagens/tamanho2.png")));	
				}else {
					peca.setIcon(new ImageIcon(tela.class.getResource("/imagens/tamanho.png")));	
				}
				*/
			...
			public void mouseClicked(MouseEvent e) {
				if(jogo_rodando==1) {
					if(e.getX()<=160 && e.getY()<=160) {
	    				...
	    				for(int i=1;i<disntancia_y;i++) {
	    					for(int j=1;j<disntancia_x;j++) {
	    						quebra_cabeca[y+linha_y*i][x+linha_x*j].setIcon(new ImageIcon(Tela.class.getResource("/imagens/pc_"+pecas.pecas[y-linha_y*(i-1)][x-linha_x*(j-1)].getI()+"_"+pecas.pecas[y-linha_y*(i-1)][x-linha_x*(j-1)].getJ()+".png")));	
		    				}
	    				}
			    		...
			    		for(int i=1;i<disntancia_y;i++) {
	    					for(int j=1;j<disntancia_x;j++) {
	    						pecas.pecas[vazio[1]-linha_y*i][vazio[0]-linha_x*j].setI(pecas.pecas[y+linha_y*((disntancia_y-1)-i)][x+linha_x*((disntancia_y-1)-j)].getI());
					    		pecas.pecas[vazio[1]-linha_y*i][vazio[0]-linha_x*j].setJ(pecas.pecas[y+linha_y*((disntancia_y-1)-i)][x+linha_x*((disntancia_y-1)-j)].getJ());
					    	}
	    				}
